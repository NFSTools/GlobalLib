        ...
        public override unsafe byte[] Assemble()
        {
            ...
            fixed (byte* byteptr_t = &result[0])
            {
                ...
                *(float*)(byteptr_t + 0x34) = this.PaintBrightness;
                ...
                // Write Generic Vinyl
                if (this._genericvinyl == Reflection.BaseArguments.NULL) { }
                else if (_genericvinyl.StartsWith("0x"))
                    *(uint*)(byteptr_t + 0x38) = Utils.ConvertX.ToUInt32(this._genericvinyl);
                else
                    *(uint*)(byteptr_t + 0x38) = Utils.Bin.Hash(this._genericvinyl);

                // Write Vector Vinyl
                if (this._vectorvinyl == Reflection.BaseArguments.NULL) { }
                else
                {
                    if (this._vectorvinyl.StartsWith("0x"))
                        *(uint*)(byteptr_t + 0x3C) = Utils.ConvertX.ToUInt32(this._vectorvinyl);
                    else
                        *(uint*)(byteptr_t + 0x3C) = Utils.Bin.Hash(this._vectorvinyl);
